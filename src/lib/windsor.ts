...
// Cache de 2h por red — protege el límite de peticiones del plan free de
// Windsor.ai. Compartida entre /api/posts y /api/analyst (mismo proceso Node).
...
export async function fetchPosts(platform: SocialPlatform): Promise<Post[]> {
  ...
  if (rows.length > 0) {
    console.log(`[windsor] (${platform}) Campos disponibles en Windsor.ai:`, Object.keys(rows[0]));
  }
