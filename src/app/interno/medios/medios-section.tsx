...
const DEFAULT_WEEK_TEMPLATE = ["Lun", "Mar", "Mie", "Jue", "Vie", "Sab", "Dom"].map((dia) => ({
  dia, notas: 0, positivas: 0, negativas: 0,
}));

const KPI_DEFS: { id: string; label: string; value: string; delta: string }[] = [
  { id: "kpi_impacto", label: "Impacto Mediático", value: "0", delta: "+0%" },
  { id: "kpi_notas", label: "Notas Publicadas", value: "0", delta: "+0%" },
  { id: "kpi_voceros", label: "Voceros Activos", value: "0", delta: "+0%" },
  { id: "kpi_alcance", label: "Alcance Radial", value: "0", delta: "+0%" },
];

function defaultKpi(id: string) {
  const def = KPI_DEFS.find((k) => k.id === id) || { id, label: id, value: "0", delta: "+0%" };
  return {
    ...def,
    sentimiento: 0,
    notas: 0,
    tendencia_semanal: id === "kpi_notas" ? DEFAULT_WEEK_TEMPLATE : [],
  };
}
...
export function MediosSection() {
  ...
  const saveMediosData = async () => {
    try {
        ...
        toast.success("Datos guardados", "Los cambios de Conversación en Medios quedaron guardados.");
        setIsEditing(false);
        fetchMediosData();
    } catch (err) {
        console.error(err);
  ...
  const handleExcelUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
        try {
            ...
            toast.success("Importación finalizada", `${profilesFound} perfiles y ${feedsFound} noticias procesadas.`);
        } catch(err) {
            console.error(err);
  ...
  const updateProfile = (id: string, patch: Record<string, any> | ((current: any) => Record<string, any>)) => {
    setProfiles(prev => {
      ...
      const current = idx !== -1 ? prev[idx] : defaultKpi(id);
  ...
  const addTopMedio = () => {
    const id = `medio_${Date.now()}`;
    setProfiles(prev => [...prev, { id, label: "Nuevo Medio", value: "0", delta: "+0%", sentimiento: 50, notas: 0, tendencia_semanal: [] }]);
  };

  const kpis = useMemo(
    () => KPI_DEFS.map((def) => profiles.find(p => p.id === def.id) || defaultKpi(def.id)),
    [profiles]
  );
  ...
  const stackedData = useMemo(() => {
    ...
    const tendencia = kpiNotas?.tendencia_semanal || DEFAULT_WEEK_TEMPLATE;
    ...
    <>
      ...
      <div className="mb-8 flex justify-between items-center">
        ...
        <div className="flex gap-3">
            ...
            <Button variant="outline" size="sm" onClick={fetchMediosData} className="panel border-[#2a2a4a] text-white">
          ...
          {kpis.map((kpi, i) => (
              <Card key={kpi.id} className="panel border-[#1e2240] p-5 rounded-2xl relative overflow-hidden group">
                  <div className="flex justify-between items-start mb-2">
                      <p className="text-[10px] font-bold text-[#8892b0] tracking-wider">
                          {isEditing ? (
                              <Input
                                value={kpi.label}
                                onChange={e => updateProfile(kpi.id, { label: e.target.value })}
                      ...
                      {isEditing ? (
                          <Input
                            value={kpi.delta}
                            onChange={e => updateProfile(kpi.id, { delta: e.target.value })}
                  ...
                  {isEditing ? (
                      <Input
                        value={kpi.value}
                        onChange={e => updateProfile(kpi.id, { value: e.target.value })}
                  ...
                  <div className="absolute bottom-0 left-0 w-full h-1 bg-[#131a30]">
                      <div
                          ...
                          style={{
                              width: '40%',
                              background: i === 0 ? '#0094ff' : i === 1 ? '#e8a817' : i === 2 ? '#2eb88a' : '#0094ff'
                          }}
                      />
                  </div>
              </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        {/* Weekly Chart */}
        <Card className="lg:col-span-2 panel border border-[#1e2240] p-6 rounded-2xl neon-frame">
            <div className="mb-8">
                <h3 className="font-bold text-lg text-[#e4e9f5]">Notas Publicadas esta Semana</h3>
                <p className="text-xs text-[#8892b0]">Distribución por sentimiento — últimos 7 días</p>
            </div>
            <div className="h-64 px-4">
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        <XAxis dataKey="dia" axisLine={false} tickLine={false} tick={{ fill: '#aab3cf', fontSize: 12 }} />
                        <YAxis axisLine={false} tickLine={false} tick={{ fill: '#aab3cf', fontSize: 11 }} />
                        <Tooltip
                            cursor={{ fill: 'rgba(255,255,255,0.03)' }}
                            contentStyle={{ backgroundColor: '#0d1120', border: '1px solid #1e2240', borderRadius: '12px' }}
                        />
            ...
            <div className="flex justify-center gap-6 mt-6 text-[10px] font-bold text-[#aab3cf] uppercase tracking-widest">
                <div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-[#2eb88a]"></span> Positivas</div>
                <div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-[#aab3cf]"></span> Neutral</div>
                <div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-[#df3a3a]"></span> Negativas</div>
            </div>

            {isEditing && (
                <div className="mt-8 pt-8 border-t border-[#1e2240] animate-in fade-in slide-in-from-top-4 duration-500">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead>
                                <tr className="text-[10px] font-bold text-[#8892b0] uppercase tracking-wider border-b border-[#2a2a4a]">
                            ...
                            <tbody>
                                {(profiles.find(p => p.id === 'kpi_notas')?.tendencia_semanal || DEFAULT_WEEK_TEMPLATE).map((day: any, dIdx: number) => {
                                    const updateDay = (field: string, value: any) => updateProfile('kpi_notas', (c) => {
                                        const trend = [...(c.tendencia_semanal || DEFAULT_WEEK_TEMPLATE)];
                                        trend[dIdx] = { ...trend[dIdx], [field]: value };
                                        return { tendencia_semanal: trend };
                                    });
                                    return (
                                        <tr key={dIdx} className="border-b border-[#1e2240] last:border-0 hover:bg-white/5 transition-colors">
                                            <td className="py-2">
                                                <Input value={day.dia} onChange={e => updateDay('dia', e.target.value)} className="h-6 w-12 bg-transparent border-none p-0 text-xs font-bold text-[#e4e9f5]" />
                                            </td>
                                            <td className="py-2">
                                                <div className="flex justify-center">
                                                    <Input type="number" value={day.notas} onChange={e => updateDay('notas', parseInt(e.target.value) || 0)} className="h-6 w-16 bg-white/5 border-[#2a2a4a] text-center text-xs text-[#75ddff] font-bold" />
                                                </div>
                                            </td>
                                            <td className="py-2">
                                                <div className="flex justify-center">
                                                    <Input type="number" value={day.positivas} onChange={e => updateDay('positivas', parseInt(e.target.value) || 0)} className="h-6 w-16 bg-green-500/5 border-green-500/20 text-center text-xs text-green-500 font-bold" />
                                                </div>
                                            </td>
                                            <td className="py-2">
                                                <div className="flex justify-center">
                                                    <Input type="number" value={day.negativas} onChange={e => updateDay('negativas', parseInt(e.target.value) || 0)} className="h-6 w-16 bg-red-500/5 border-red-500/20 text-center text-xs text-red-500 font-bold" />
                                                </div>
                                            </td>
                                            <td className="py-2 text-right text-[10px] text-[#8892b0] font-mono font-bold">
        ...
        {/* Top Media */}
        <Card className="panel border border-[#1e2240] p-6 rounded-2xl overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
                <h3 className="text-sm font-semibold text-[#e4e9f5] uppercase tracking-widest">Top Medios</h3>
                {isEditing && (
                    <Button size="sm" variant="ghost" className="text-[10px] font-black uppercase text-[#75ddff] h-6 px-2" onClick={addTopMedio}>
                        <FontAwesomeIcon icon={faPlus} className="mr-1" /> Agregar
                    </Button>
                )}
            </div>
            {topMedios.length === 0 && !isEditing && (
                <p className="text-xs text-[#8892b0] italic">Sin medios registrados aún. Entra a Modo Edición para agregar el primero.</p>
            )}
            <div className="space-y-4">
                {topMedios.map((m, i) => (
                    <div key={m.id} className="flex items-center gap-3 group relative">
                        <div className="shrink-0 w-5 h-5 rounded flex items-center justify-center text-[10px] font-black" style={i < 3 ? { color: '#fbbf24' } : { color: '#131a30' }}>
                            {i + 1}
                        </div>
                        <div className="flex items-center gap-2 w-32 shrink-0">
                            <FontAwesomeIcon icon={getMediaIcon(m.label)} className="w-3.5 h-3.5 text-[#0094ff]" />
                            {isEditing ? (
                                <Input
                                    value={m.label}
                                    onChange={e => {
                                    ...
                                    className="h-6 text-xs font-bold bg-white/5 border-[#2a2a4a] p-1"
                                />
                            ) : (
                                <span className="text-xs font-bold truncate text-[#ffffff]">{m.label}</span>
                            )}
                        </div>
                        <div className="flex-1 space-y-1">
                            <div className="flex justify-between text-[9px] font-bold">
                                {isEditing ? (
                                    <div className="flex gap-1">
                                        ...
                                        <span className="text-[#8892b0]">notas</span>
                                    </div>
                                ) : (
                                    <span className="text-[#8892b0]">{m.notas} notas</span>
                                )}
                            ...
                            <div className="h-1 w-full bg-[#131a30] rounded-full overflow-hidden">
                                <div className="h-full bg-[#0094ff]" style={{ width: `${(m.notas / (topMedios[0]?.notas || 1)) * 100}%` }} />
                            </div>
                        </div>
                        {isEditing && (
      ...
      <div className="panel border border-[#1e2240] rounded-2xl p-6 mb-20">
        <div className="flex items-center justify-between mb-6">
            ...
            {isEditing && (
                <Button size="sm" variant="ghost" onClick={() => setFeed([{ medio: 'Medio', tiempo: new Date().toISOString(), texto: '', tipo: 'neutral' }, ...feed])} className="text-[10px] font-black uppercase text-[#75ddff]">
        ...
        <div className="space-y-2">
            {feed.map((post, idx) => (
                <div key={post.id || idx} className="well border border-[#1e2240] rounded-xl p-4 flex items-center gap-4 hover:border-[#2a2a4a] transition-colors group relative">
                    <div className="p-2 rounded-lg bg-white/5 text-[#75ddff]">
                            ...
                            {isEditing ? (
                                <>
                                    <Input value={post.medio} onChange={e => {
                                        ...
                                        setFeed(up);
                                    }} className="h-6 text-xs font-bold text-[#75ddff] bg-white/5 border-[#2a2a4a] w-32" />
                                    <Input
                                        type="datetime-local"
                                        ...
                                        className="h-6 text-xs bg-white/5 border-[#2a2a4a] w-44"
                                    />
                                    <select
                                        value={post.tipo}
                                        onChange={e => {
                                        ...
                                        className="bg-[#131a30] text-[10px] font-bold rounded p-1 border-none outline-none"
                                    >
                                ...
                                <>
                                    <span className="font-bold text-sm text-[#75ddff]">{post.medio.toUpperCase()}</span>
                                    <span className="text-xs text-[#8892b0]">{formatTime(post.tiempo)}</span>
                                </>
                            )}
                        </div>
                        {isEditing ? (
                            ...
                            <p className="text-sm text-[#c0c8de] font-medium group-hover:text-white transition-colors">{post.texto}</p>
                        )}
                    </div>
                    {isEditing ? (
                        ...
                        <div className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${post.tipo === 'positivo' ? 'bg-green-500/10 text-green-500' : post.tipo === 'negativo' ? 'bg-red-500/10 text-red-500' : 'bg-[#8892b0]/10 text-[#8892b0]'}`}>
                            {post.tipo}
                        </div>
                    )}
                </div>
      ...
      <AdminPopup title="Editor de Conversación en Medios" hideTrigger={true}>
            <div className="space-y-6">
                <div className="flex justify-between items-center bg-[#131a30] p-4 rounded-xl border border-[#1e2240]">
                    <div>
                        <h3 className="font-bold">Panel de Administración</h3>
                        <p className="text-xs text-[#aab3cf]">Edita métricas, gráficos y titulares del día.</p>
                    </div>
                    <div className="flex gap-2">
                        ...
                        <Button className="bg-[#0094ff] hover:bg-[#0080e6] font-bold" onClick={saveMediosData}>
                ...
                <div className="grid grid-cols-1 gap-8">
                    ...
                    <div>
                        <h3 className="text-[#75ddff] font-black mb-4 uppercase text-xs tracking-widest flex items-center gap-2">
                            <span className="w-2 h-2 rounded-full bg-[#0094ff]"></span> KPIs y Medios Top
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {profiles.map((p, idx) => (
                                <div key={p.id} className="bg-white/5 p-4 rounded-xl space-y-3 border border-[#1e2240]">
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="font-bold uppercase text-[10px] text-[#aab3cf]">{p.id}</span>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="space-y-1">
                                            <label className="text-[9px] text-[#8892b0] uppercase font-black">Valor/Texto</label>
                                        ...
                                        <div className="space-y-1">
                                            <label className="text-[9px] text-[#8892b0] uppercase font-black">Delta/Positiva</label>
                                        ...
                                        <div className="space-y-1">
                                            <label className="text-[9px] text-[#8892b0] uppercase font-black">Notas Totales</label>
                    ...
                    <div>
                        ...
                        <div className="bg-white/5 p-4 rounded-xl border border-[#1e2240] overflow-x-auto">
                            <table className="w-full text-xs text-left">
                                <thead>
                                    <tr className="text-[#8892b0] border-b border-[#2a2a4a]">
                                        <th className="pb-2">Día</th>
                                ...
                                <tbody>
                                    {(() => {
                                        ...
                                        return kpiNotas.tendencia_semanal.map((day: any, dIdx: number) => (
                                            <tr key={day.dia} className="border-b border-[#1e2240] last:border-0">
                                                <td className="py-2">
                                                ...
                                                <td className="py-2">
                                                    <Input type="number" value={day.notas} onChange={e => {
                                                        ...
                                                        up[pIdx].tendencia_semanal = newTrend;
                                                        setProfiles(up);
                                                    }} className="h-7 w-20 well border-[#1e2240] px-2" />
                                                </td>
                                                <td className="py-2">
                                                    <Input type="number" value={day.positivas} onChange={e => {
                                                        ...
                                                        up[pIdx].tendencia_semanal = newTrend;
                                                        setProfiles(up);
                                                    }} className="h-7 w-20 well border-[#1e2240] px-2 text-green-500" />
                                                </td>
                                                <td className="py-2">
                                                    <Input type="number" value={day.negativas} onChange={e => {
                                                        ...
                                                        up[pIdx].tendencia_semanal = newTrend;
                                                        setProfiles(up);
                                                    }} className="h-7 w-20 well border-[#1e2240] px-2 text-red-500" />
                                                </td>
                                                <td className="py-2 text-right text-[#aab3cf] font-mono">
                                                    {day.notas - day.positivas - day.negativas}
                                                </td>
                                            </tr>
                                        ));
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <Button size="sm" variant="ghost" className="text-[10px] uppercase font-black text-[#aab3cf] hover:text-white" onClick={() => setFeed([{ medio: 'Medio', tiempo: new Date().toISOString(), texto: '', tipo: 'neutral' }, ...feed])}>
                                    ...
                                    <div className="flex-1 space-y-2">
                                        <div className="flex gap-2">
                                            <Input value={post.medio} placeholder="Medio" onChange={(e) => {
                                                ...
                                                setFeed(news);
                                            }} className="well border-[#1e2240] h-8 text-xs w-1/3" />
                                            <Input
                                                type="datetime-local"
                                                value={post.tiempo && !isNaN(new Date(post.tiempo).getTime()) ? new Date(new Date(post.tiempo).getTime() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16) : ''}
                                                onChange={(e) => {
                                            ...
                                            <select className="well border-[#1e2240] h-8 text-xs rounded-md px-2 w-1/3" value={post.tipo} onChange={(e) => {
                                                const news = [...feed];
                                        ...
                                        <textarea className="w-full well border-[#1e2240] rounded-md p-2 text-xs text-white focus:outline-none" value={post.texto} rows={2} onChange={(e) => {
                                            const news = [...feed];
